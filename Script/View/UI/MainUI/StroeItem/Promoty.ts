...
@ccclass('Promoty')
export class Promoty extends IStroeItem {
    @property(CCInteger)
    promotyCount: number = 0
    ...
    revieveFunc() {
        var callback = function (isEnd) {
            if (isEnd) {
                ...
                gameInfo.promotyCount += this.promotyCount
                gameInfo.hadSeeAd = 1
                ...
                this.recieved = true
                let data = {
                    label: this.promotyCount.toString()
                }
                DialogManager.getInstance().showDlg("PromotyDialog", data)
            }
            else {
